...
const COURSE = [
  ...
  },
  {
    ...
    topics: [
      ...
      "Custom Compose views and modifiers",
      "Architecting a maintainable screen",
    ],
  },
  {
    id: "s8",
    title: "Testing",
    built: true,
    topics: ["Unit Test with Jest", "E2E with Maestro"],
  },
  {
    id: "s9",
  ...
  },
  {
    ...
    title: "EAS Submit",
    built: true,
    topics: ["Submit to App Store", "Submit to Google Play"],
  },
  {
    id: "s14",
    title: "EAS Update",
  ...
  },
  {
    ...
    title: "Publishing",
    built: true,
    topics: ["App Store Submission", "Play Store Submission"],
  },
  {
    id: "s17",
    title: "Payments",
    built: true,
    topics: ["Payment Basics", "In-app Purchases"],
  },
  {
    id: "s18",
    ...
    sec.topics.forEach((t) => {
      ...
      if (el)
        pageItems.push({
          ...
          secTitle: sec.title,
          secIndex:
            COURSE.filter((s) => s.built).findIndex((x) => x.id === sec.id) + 1,
        });
    });
  });
...
function runFetchDemo() {
  ...
  screen.innerHTML =
    "<span style=\"font-family:'JetBrains Mono',monospace; font-size:12px; color:#5c6070;\">Loading…</span>";
  fetch("https://jsonplaceholder.typicode.com/posts?_limit=3")
    ...
    .then((data) => {
      ...
      wrap.innerHTML =
        `<div style="font-family:'Inter',sans-serif;font-size:12px;color:#5c6070;margin-bottom:2px;">Loaded ${data.length} posts!</div>` +
        data
          .map(
            (p) =>
              `<div style="background:#f2f0ea;border-radius:8px;padding:8px 10px;font-family:'Inter',sans-serif;font-size:11.5px;color:#1a1d24;">${p.title.slice(0, 40)}…</div>`,
          )
          .join("");
    ...
    .catch(() => {
      screen.innerHTML =
        "<span style=\"font-family:'Inter',sans-serif;font-size:12px;color:var(--coral);\">Network error — try again</span>";
    });
}
...
const supabaseTabContent = {
  table:
    "<b>Table Editor</b><br><br>A spreadsheet-like grid view of every table in your database. Add columns, edit rows, and set constraints visually — every change here is really just running SQL behind the scenes.",
  ...
  storage:
    "<b>Storage</b><br><br>Upload and serve files — profile photos, attachments, videos — similar to S3-style object storage, with access rules that can mirror your database's RLS policies.",
  realtime:
    "<b>Realtime</b><br><br>Subscribe to live changes on a table. When a row is inserted, updated, or deleted, subscribed clients receive the change instantly, without polling.",
};
...
function renderTableBuilder() {
  ...
  if (!list || !sql) return;
  list.innerHTML = tableBuilderCols
    .map(
      (c) =>
        '<div style="display:flex; justify-content:space-between; padding:6px 10px; background:var(--paper-dim); border-radius:6px; font-family:var(--font-mono); font-size:11.5px;"><span>' +
        c.name +
        '</span><span style="color:var(--text-mute);">' +
        c.type +
        "</span></div>",
    )
    .join("");
  const lines = tableBuilderCols.map(
    (c, i) =>
      "  " +
      c.name +
      " " +
      c.type +
      (i < tableBuilderCols.length - 1 ? "," : ""),
  );
  sql.querySelector("code").textContent =
    "CREATE TABLE todos (\n" + lines.join("\n") + "\n);";
}
...
function crudRenderList() {
  ...
  if (!list) return;
  list.innerHTML = crudRows
    .map(
      (row) =>
        '<div style="display:flex; align-items:center; gap:8px; background:#f2f0ea; border-radius:8px; padding:8px 10px;">' +
        '<input type="checkbox" ' +
        (row.done ? "checked" : "") +
        ' onchange="crudToggleRow(' +
        row.id +
        ')">' +
        "<span style=\"flex:1; font-family:'Inter',sans-serif; font-size:12px; color:#1a1d24; text-decoration:" +
        (row.done ? "line-through" : "none") +
        ';">' +
        row.text +
        "</span>" +
        '<span style="cursor:pointer; color:var(--coral); font-size:13px;" onclick="crudDeleteRow(' +
        row.id +
        ')">✕</span>' +
        "</div>",
    )
...
const buildProfileDetails = {
  development:
    "<b>Development:</b> Includes dev-client tooling, installs directly on your own registered device, used for day-to-day feature work with live JS reload.",
  preview:
    "<b>Preview:</b> Release-like build (no debug tooling), distributed via a shareable install link — perfect for QA and stakeholder review before a real release.",
  production:
    "<b>Production:</b> The final, store-ready artifact submitted to the App Store or Play Store, typically with autoIncrement enabled for its build number.",
};
...
/* ---------- Eye-safe theme toggle (persisted) ---------- */
(function () {
  const key = "eyeSafeTheme";
  const btn = document.getElementById("eyeToggleBtn");
  function apply(val) {
    document.documentElement.classList.toggle("eye-safe", !!val);
    if (btn) btn.textContent = val ? "Eye-Safe ✓" : "Eye-Safe";
    if (btn) btn.setAttribute("aria-pressed", !!val);
  }
  try {
    const pref = localStorage.getItem(key) === "1";
    apply(pref);
  } catch (e) {
    /* ignore */
  }
  if (btn) {
    btn.addEventListener("click", () => {
      const now = !document.documentElement.classList.contains("eye-safe");
      try {
...
function runAppStorePipeline() {
  ...
  function step() {
    if (i > 0) {
      stages[i - 1].style.background = "var(--teal-dim)";
      stages[i - 1].style.borderColor = "var(--teal)";
    }
    if (i < stages.length) {
      stages[i].style.background = "var(--coral-dim)";
      stages[i].style.borderColor = "var(--coral)";
...
const playTrackDetails = {
  internal:
    "<b>Internal testing:</b> Up to 100 hand-picked testers, updates appear almost instantly, no Google review required — the fastest feedback loop.",
  closed:
    "<b>Closed testing:</b> Specific groups or email lists you invite, with a lighter review than full production.",
  open: "<b>Open testing:</b> Anyone can opt in via a public link, subject to Google's full review process.",
  production:
    "<b>Production:</b> All Play Store users, released either fully or via a staged rollout percentage.",
};
...
function updateRolloutDemo(val) {
  ...
  if (val == 0) {
    explain.textContent =
      "At 0%, no users receive the new version yet — the rollout is effectively paused.";
  } else if (val == 100) {
    explain.textContent =
      "At 100%, every user updating now gets the new version — the full rollout is complete.";
  } else {
    explain.textContent =
      "~" +
      val +
      " out of every 100 users updating right now get the new version; the rest stay on the previous one until you increase the percentage.";
  }
}
    ...
    setTimeout(() => {
      ...
      const interval = setInterval(() => {
        if (i < devices.length) {
          ...
          status.textContent =
            "All connected devices will receive this on their next cold start.";
          btn.disabled = false;
...
let updateHistory = [
  ...
  {
    id: "grp_001",
    message: "Initial release update",
    date: "1 week ago",
    current: false,
  },
];
function renderUpdateHistory() {
  ...
  list.innerHTML = updateHistory
    .map(
      (u) =>
        '<div style="display:flex; align-items:center; justify-content:space-between; gap:10px; padding:12px 14px; border-radius:10px; border:1px solid ' +
        (u.current ? "var(--teal)" : "var(--line)") +
        "; background:" +
        (u.current ? "var(--teal-dim)" : "var(--card)") +
        ';">' +
        '<div><div style="font-family:var(--font-mono); font-size:11px; color:var(--text-mute);">' +
        u.id +
        " · " +
        u.date +
        (u.current ? ' · <b style="color:var(--teal);">CURRENT</b>' : "") +
        "</div>" +
        ...
        (u.current
          ? ""
          : '<button class="demo-btn ghost update-revert-btn" style="white-space:nowrap;" data-revert-id="' +
            u.id +
...
function revertToUpdate(id) {
  ...
  updateHistory.unshift({
    id: "grp_00" + (updateHistory.length + 1),
    message: "Reverted to: " + target.message,
    date: "Just now",
    current: true,
  });
  renderUpdateHistory();
...
function runWorkflowTrigger() {
  ...
  btn.disabled = true;
  jobs.forEach((j) => {
    j.style.background = "var(--card)";
    j.style.borderColor = "var(--line)";
  });
  const messages = [
    "Push to main detected by EAS Workflows.",
    "Running job: build_ios (profile: preview)…",
  ];
  let i = 0;
  function step() {
    if (i > 0) {
      jobs[i - 1].style.background = "var(--teal-dim)";
      jobs[i - 1].style.borderColor = "var(--teal)";
    }
    if (i < jobs.length) {
      ...
      status.textContent =
        "Workflow complete — build_ios finished successfully.";
      btn.disabled = false;
    }
  }
  step();
...
function runReleaseWorkflow() {
  ...
  btn.disabled = true;
  status.textContent =
    "Tag v2.3.0 pushed — starting both platform chains in parallel…";
  document.querySelectorAll(".rel-job").forEach((j) => {
    j.style.background = "var(--card)";
    j.style.borderColor = "var(--line)";
  });
  ...
  function checkAllDone() {
    ...
    if (doneCount === 2) {
      status.textContent =
        "Both platforms submitted — release v2.3.0 is on its way to review.";
      btn.disabled = false;
    }
  }
...
function buildChecklist(containerId, items, progressId, fillId) {
  ...
  container.innerHTML = items
    .map(
      (text, i) =>
        ...
        '<input type="checkbox" class="checklist-cb" data-idx="' +
        i +
        '">' +
        "<span>" +
        text +
  ...
  container.querySelectorAll(".checklist-cb").forEach((cb) => {
    cb.addEventListener("change", () =>
      updateChecklistProgress(containerId, items.length, progressId, fillId),
    );
  });
}
...
const paymentAnswers = {
  digital:
    "<b>Use In-App Purchase (RevenueCat).</b><br><br>Since this unlocks features or content inside the app, Apple and Google require their own purchase systems — Stripe isn't allowed here, regardless of preference.",
  physical:
    "<b>Use Stripe.</b><br><br>Physical goods are exempt from in-app purchase requirements. Stripe gives you full payment method flexibility with no platform commission.",
  service:
    "<b>Use Stripe.</b><br><br>Real-world services consumed outside the app (rides, bookings, deliveries) are also typically exempt — this is exactly the case Stripe was built for.",
};
...
const nativeLayerDetails = {
  ...
  module:
    "<b>3. Expo Module:</b> Your actual Swift (iOS) or Kotlin (Android) function runs, registered and exposed through the Expo Modules API.",
...
function renderNmDecision() {
  ...
  if (!container) return;
  container.innerHTML = nmDecisionQuestions
    .map(
      (text, i) =>
        '<label style="display:flex; align-items:center; gap:10px; padding:8px 4px; font-size:13px; cursor:pointer; color:var(--text);">' +
        '<input type="checkbox" class="nm-decision-cb" data-idx="' +
        i +
        '">' +
        "<span>" +
        text +
...
function updateNmDecisionResult() {
  ...
  if (checked === nmDecisionQuestions.length) {
    result.innerHTML =
      '<b style="color:var(--teal);">Looks justified.</b> All boxes checked — a custom native module is a reasonable path here.';
  } else if (checked === 0) {
    result.textContent = "Check items above that are true for your situation.";
  } else {
    result.innerHTML =
      '<b style="color:var(--coral);">Not quite yet.</b> ' +
      (nmDecisionQuestions.length - checked) +
      " item(s) unchecked — worth reconsidering an existing library or Expo UI component first.";
  }
}
...
renderUpdateHistory();
buildChecklist(
  "appStoreChecklistItems",
  appStoreItemsList,
  "appStoreProgress",
  "appStoreProgressFill",
);
buildChecklist(
  "playStoreChecklistItems",
  playStoreItemsList,
  "playStoreProgress",
  "playStoreProgressFill",
);
renderNmDecision();
